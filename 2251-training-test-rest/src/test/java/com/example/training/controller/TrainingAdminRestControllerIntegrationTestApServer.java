...
class TrainingAdminRestControllerIntegrationTestApServer {
    ...
    void test_getTraining() {
        ResponseEntity<Training> responseEntity = testRestTemplate.getForEntity("/api/trainings/{id}", Training.class,
                "t01");
        Assertions.assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
        Training training = responseEntity.getBody();
        Assertions.assertThat(training.getId()).isEqualTo("t01");
