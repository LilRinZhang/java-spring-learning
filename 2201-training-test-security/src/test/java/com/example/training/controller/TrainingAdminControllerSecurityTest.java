...
class TrainingAdminControllerSecurityTest {
    ...
    void test_displayList_GUESTユーザはアクセスできない() throws Exception {
        mockMvc.perform(
                get("/admin/training/display-list").with(user("foo").roles("GUEST")))
    ...
    void test_displayList_STAFFユーザはアクセスできる() throws Exception {
        mockMvc.perform(
                get("/admin/training/display-list").with(user("foo").roles(
                        "STAFF")))
    ...
    void test_validateUpdateInput_STAFFユーザはアクセスできない() throws Exception {
        mockMvc.perform(
                post("/admin/training/validate-update-input").with(user("foo").roles(
                        "STAFF"))
                        .with(csrf()))
                .andExpect(status().isForbidden());
    }
